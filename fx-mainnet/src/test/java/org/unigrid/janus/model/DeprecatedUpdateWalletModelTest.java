...
public class DeprecatedUpdateWalletModelTest extends BaseMockedWeldTest {
	...
	public boolean shouldReturnTrueOnCheckUpdateBootstrap() {
		...
		String[] newVersionArray = newVersion.split("_")[0].split(".");

		for (String version : newVersionArray) {
			if (version.length() == 2) {
				ignore = false;
			}
		}

		String[] currentVersionArray = currentVersion.split("_")[0].split(".");
		for (String version : currentVersionArray) {
			if (version.length() == 2) {
				ignore = false;
			}
		}
