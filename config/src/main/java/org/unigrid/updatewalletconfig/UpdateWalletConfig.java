...
public class UpdateWalletConfig extends AbstractMavenLifecycleParticipant {
    ...
    public void afterSessionEnd(MavenSession mavenSession) throws MavenExecutionException {
        ...
        configuration.setFiles(allFiles);
    ...
    public void generateUpdateConfigFile(MavenProject fx, MavenProject bootstrap, OS os, boolean testing, String chain) throws MavenExecutionException {
        // Retrieve the opens and exports configurations from the fx project properties
        ...
        // Use a set to track added URIs and avoid duplication
        Set<String> addedUris = new HashSet<>();
        List<FileMetadata> filesToUse = new ArrayList<>();
    
        //System.out.println("Initial allFiles size: " + allFiles.size());
        //System.out.println("Initial testingFiles size: " + testingFiles.size());
        //System.out.println("Files to use (before adding): " + filesToUse.size());
    
        List<FileMetadata> selectedFiles = testing ? testingFiles : allFiles;
    
        for (FileMetadata file : selectedFiles) {
            if (isValidForChain(file, chain) && addedUris.add(file.getUri())) {
                filesToUse.add(file);
                // Add dependencies recursively for each FX file
                List<FileMetadata> dependencies = getDependencies(getFxDependencyString(fx));
                for (FileMetadata dependency : dependencies) {
                    if (addedUris.add(dependency.getUri())) {
        ...
        if (testing) {
            // Add testing daemon and hedgehog files
            addTestingDaemonAndHedgehogFiles(filesToUse, addedUris, os, chain);
        } else {
            // Add mainnet daemon and hedgehog files only if the chain is legacy
            addProductionDaemonAndHedgehogFiles(filesToUse, addedUris, os, chain);
        }        
    
        //System.out.println("Files to use (after adding): " + filesToUse.size());
        // for (FileMetadata file : filesToUse) {
        //     System.out.println("Added file: " + file.getUri());
        // }
    
        // Add opens and exports packages for specific files
        for (FileMetadata file : filesToUse) {
        ...
        // Debugging: Print the configuration before marshalling
        //System.out.println("Configuration before marshalling: " + configuration);
    ...
    private void addTestingDaemonAndHedgehogFiles(List<FileMetadata> filesToUse, Set<String> addedUris, OS os, String chain) {
        ...
        // we ignore adding the daemon file for the mainnet chain
    ...
    public List<FileMetadata> getExternalDependencies(File baseDir, String chain, boolean isTesting) throws MavenExecutionException {
        ...
        try {
            ...
            // Add Daemon and Hedgehog URLs (unchanged)
            if (chain.equals("legacy")) {
        ...
        // Debugging: Print the list of external dependencies
